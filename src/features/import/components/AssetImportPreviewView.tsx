    ...
    return (
        <div className="w-full space-y-6 max-w-7xl mx-auto pb-12 px-4 sm:px-6 lg:px-8 text-[#1C1C1E]">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-200 pb-4">
                <div className="flex items-center gap-3 sm:gap-4 text-xs overflow-x-auto py-1 font-medium">
                    <div className="flex items-center gap-2 text-neutral-400 shrink-0">
                        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-neutral-100 text-neutral-600 text-[10px]">✓</span>
                        <span>1. Unggah file</span>
                    </div>
                    <span className="text-neutral-300">—</span>

           
                    <div className="flex items-center gap-2 text-neutral-900 font-semibold shrink-0">
                        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-neutral-900 text-white text-xs font-bold">2</span>
                        <span>2. Pratinjau & koreksi</span>
                    </div>
                    <span className="text-neutral-300">—</span>

          
                    <div className="flex items-center gap-2 text-neutral-400 shrink-0">
                        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-neutral-100 text-neutral-600 text-xs">3</span>
                        <span>3. Simpan</span>
                    </div>
                </div>
                ...
                <button 
                    ...
                    className="text-xs font-medium text-neutral-600 hover:text-neutral-900 text-left sm:text-right"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="rounded-2xl border border-[#A7F3D0] bg-[#ECFDF5] p-5 space-y-1 shadow-sm">
                ...
                <div className="rounded-2xl border border-[#FECDD3] bg-[#FFF1F2] p-5 space-y-1 shadow-sm">
                ...
                <div className="rounded-2xl border border-[#FDE68A] bg-[#FFFBEB] p-5 space-y-1 shadow-sm">
                ...
                <div className="rounded-2xl border border-neutral-200 bg-white p-5 space-y-1 shadow-sm">
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm overflow-hidden">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 border-b border-neutral-200 gap-3">
                    <p className="text-xs font-medium text-neutral-700">
                        <span className="font-bold text-rose-600">{stats.gagal} baris gagal</span> — Koreksi langsung di tabel ini, atau lanjutkan tanpa baris tersebut.
                    </p>
                            ...
                            {filteredData.length > 0 ? (
                                filteredData.map((row) => (
                                    <tr key={row.id} className="hover:bg-neutral-50/50 transition-colors">
                                        ...
                                        <td className={`py-4 px-6 font-mono ${row.dibacaJadi.includes('-') ? 'text-rose-600' : 'text-neutral-700'}`}>{row.dibacaJadi}</td>
                                        <td className="py-4 px-6 text-rose-600 font-medium">{row.alasanGagal}</td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-[#FDE68A] bg-[#FFFBEB] p-5 text-xs text-[#78350F] shadow-sm">
                <div>
